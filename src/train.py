...
def _step(
        ...
        Dh: nn.Module,
        optimD: nn.Optimizer,
        optimG: nn.Optimizer) -> Tuple[int,int]:
    fake_z = Gz(h)
    fake_h = Gh(z)
    id_h = Gh(h)
    id_z = Gz(z)
    ...
    G_loss = square_loss_gz + cycle_consistency_loss_gz + identity_loss_gz + square_loss_gh + cycle_consistency_loss_gh + identity_loss_gh
    G_loss.backward()
    optimG.step()
    # D loss
...
def train(
        ...
        num_epochs: int=5) -> Tuple[list[int],list[int],list[np.array]]:
    # Lists to keep track of progress
    img_list = []
    G_losses = []
    D_losses = []
    iters = 0
    optimD = optim.Adam(itertools.chain(Dz.parameters(), Dh.parameters()), lr=lr, betas=(beta1, 0.999))
    optimG = optim.Adam(itertools.chain(Gz.parameters(), Gh.parameters()), lr=lr, betas=(beta1, 0.999))
    ...
    for epoch in range(num_epochs):
        ...
        for i, (h,z) in enumerate(loop):
            ...
            iters += 1
        print(f"Epoch {epoch+1}/{num_epochs}: G_loss: {(epoch_g_loss/i+1):.4f}, D_loss: {(epoch_d_loss/i+1):.4f}")
...
def fake_vs_real(dataloader: tud.Dataset, img_list: list[np.array], device: Any) -> None:
...
if __name__ == "__main__":
    dataloader = data.get_datasets("dataset/horse2zebra/trainA/", "dataset/horse2zebra/trainB/")
